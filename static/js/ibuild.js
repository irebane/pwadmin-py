...
function getPItemData(e){
	...
	if ((cat == "1")||(cat == "2")||(cat == "3")){
		...
		if (sct==1){
			...
			stnDesc = stnDesc+"<font color='#4ade80'>Armor:</font> <font color='#007'>"+GetAddonString(stnAddTxt[1][0],stnAddTxt[1][1])+"</font><br>";
...
function UpdateSelItemChanges(){
	...
	document.getElementById("MailResponse").innerHTML = "<font color='#4ade80'><b><i> Item info updated! </i></b></font>";
}
...
function refreshSelectedItems(){
	...
	if (SIInd == 0){
		...
		for (i = 1; i <= SIInd; i++) {
			myArr=SIData[i].split("#");
			txt = txt+"<div style='display:flex;align-items:center;gap:4px;padding:2px 0;'><input type='checkbox' style='flex:none;' value='"+SIData[i]+"' id='SelItmChBox"+i+"' onClick=SelChckboxToggle('');><a style='flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;' title='Role Id: "+myArr[0]+"\nMoney: "+myArr[1]+"\nTitle: "+myArr[2]+"\nMessage: "+myArr[3]+"\nItemId: "+myArr[7]+"\nItem Mask:"+myArr[8]+"\nProcType: "+myArr[9]+"\nStack: "+myArr[10]+"/"+myArr[11]+"\nGuid1: "+myArr[12]+"\nGuid2: "+myArr[13]+"\nExpire: "+myArr[14]+"\nOctet: "+myArr[15]+"\n\nClick: Select or Unselect\nDouble click: load the item octet for edit' onClick=SelChckboxToggle('"+i+"'); ondblclick='OctSelectedItem("+i+", 1);'><b>"+myArr[5]+"</b> - "+myArr[6]+"</a><span style='flex:none;display:flex;gap:2px;'><a href='javascript:void(0);' onclick='EditSelectedItem("+i+");'><button> Nfo </button></a><a href='javascript:void(0);' onclick='SendSelectedItem("+i+");'><button> Send </button></a><a href='javascript:void(0);' onclick='RemoveSelectedItem("+i+");'><button> Del </button></a></span></div>";
		}
		div.innerHTML = txt;
	}
}
		...
		if (tmp != "0"){
			if (tmp.indexOf("#") != -1){
				...
				if ((isNumber(tApt))&&(isNumber(tRat))){
					...
					txt=txt+"<font color='#4ade80'>Birthstar:</font> "+GetAddonString(bStarA[x][2], val)+"<br>";
				}
			}
		}
	}	
...
function LoadPacketListHandler(userData){
	...
	if (userD[0]["error"]!=""){
		...
		refreshSelectedItems();
		document.getElementById('MailResponse').innerHTML = "<font color='#4ade80'><b><i> "+SIInd+" packet loaded from file! </i></b></font>";
	}		
}
...
function roleIdHandler(userData){
	...
	if (userD[0]["error"]!=""){
		...
		if (userD[0]["roleid"]!=""){
			...
			document.getElementById('MailResponse').innerHTML = '<font color="#4ade80"><b><i> For this name the result is: '+userD[0]["roleid"]+'! </i></b></font>';
		}
	}	
}
...
function SendCustomItem(){
	var accSel=document.getElementById("send-account");
	var charSel=document.getElementById("send-char");
	if(!accSel||!accSel.value||!charSel||charSel.disabled||!charSel.value){
		var resp=document.getElementById("MailResponse");
		if(resp){resp.style.color="#f87171";resp.textContent="Please select recipient";}
		return;
	}
...
function ItemDataModWithAjax(typ, obj) {
	...
	if (typ==1){
		xmlhttp.open("POST", "/api/characters/load", false);
	}else if (typ==3){
		xmlhttp.open("POST", "/api/items/log", false);
	}else if (typ==7){
		xmlhttp.open("POST", "/api/items/save", false);
	}else if (typ==8){
		xmlhttp.open("POST", "/api/game/packets", false);
	}
